...
creategreenlight list for symbols etc, see max passthrough for api call interms of usd.
try adding timestamp to url
...
success_coins = []
requested_symbols =[]
hardcoded_symbols = 'BTC,ETH,LTC,BCH,RPX,DASH,ZCASH'
api = "cryptocompare"
loop_delay = 10
    ...
    for symbol in symbol_list:
        ...
        if symbol not in error_coins:
            ...
            if  "Response" not in keys:
                ...
                test_df = test_df.assign (coin = symbol,coin_units = 1, timestamp_api_call = dt.datetime.now(),computer_name = 'JordanManual',api = api ) ##replace with ec2ip/region
                #print test_df
