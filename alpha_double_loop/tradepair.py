...
class GetTradePair(object):
    ...
    def trading_partners(self,symbol,error_symbols, frames):
                ...
                if data["Data"] != [] and data["Response"] == "Success":
                    ...
                    if not df.empty:
                        ...
                        for exchange in x:
                            ...
                            trade_pair.append({symbol:x})
                            dfs = []
                            dfs.append(df)
                            df = p.concat(dfs)

                            print self.exchange_trade_pair
                            self.exchange_trade_pair.append({raw_exchange:trade_pair})
    ...
    def main(self):
        ...
        try:
            ...
            for symbol_list in tqdm(xsymbols,desc='trading_partners'):
                ...
                threads = [threading.Thread(target=gtp.trading_partners, args=(symbol,error_symbols,frames,)) for symbol in symbol_list]
...
if __name__ == '__main__':
    ...
    runner = GetTradePair(symbols,50,cwd,trade_pair,exchanges,exchange_trade_pair)
    #runner = GetTradePair()
    runner.main()
